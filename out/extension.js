...
const fs = require("fs");
const Project_Structure_Analysis_1 = require("./Services/Project_Structure_Analysis");
const CodeQualityAnalysis_1 = require("./Services/CodeQualityAnalysis");
const PromptGenerator_1 = require("./Services/PromptGenerator");
const Apianalyzer_1 = require("./Services/Apianalyzer");
if (!RegExp.escape) {
...
class FlutterAppAnalyzer {
    ...
    constructor() {
        ...
        this.apiAnalyzer = new Apianalyzer_1.ApiAnalyzer();
        this.analysisReport = "";
    }
  ...
  `;
    ...
    displayLoadingWebview() {
        // Create and show a loading webview panel
        this.webViewPanel = vscode.window.createWebviewPanel("loading", "Loading...", vscode.ViewColumn.One, {});
  ...
  `;
    ...
    async analyzeFiles(workspaceFolder) {
        ...
        const promptGenerator = new PromptGenerator_1.PromptGenerator({
            ...
            apiCallingFunctions: Array.from(this.apiAnalyzer.getApiCallingFunctions()),
    ...
    getWebviewContent() {
      ...
      `;
            ...
            .join("");
        const apiCallingFunctions = Array.from(this.apiAnalyzer.getApiCallingFunctions())
            .map((apiFunction) => this.formatFunction(apiFunction))
            .join("<br>");
        const apicatchfunction = Array.from(this.apiAnalyzer.getErrorHandlingFunctions())
...
<br>
 ...
 <h2>2. Project-Structure Analysis</h2>
  ...
  `;
    ...
    async runFeatureAnalysis() {
        ...
        if (workspaceFolder) {
            ...
            try {
                ...
                const promptAnswer = await promptGenerator.generateExtensionPrompt(fileContent);
    ...
    createFeatureWebviewPanel(featurePrompt) {
        // Create and show a new webview panel for the feature analysis
        const featureWebViewPanel = vscode.window.createWebviewPanel("Promptgeneration", "Prompt Generation", vscode.ViewColumn.One, {});
        // Set the webview content
    ...
    getFeatureWebviewContent(featurePrompt) {
...
function activate(context) {
    ...
    let analysisdisposable = vscode.commands.registerCommand("extension.analyzeFlutterApp", () => {
        analyzer.runAnalysis();
    });
    // command for creating a new feature
    let createFeatureDisposable = vscode.commands.registerCommand("extension.createFlutterFeature", () => {
